...
d = @(t) zeros(size(t));
[t, z] = ode45(@(t, z) lyapunov_nonlinear(t, z, u, C, theta_star, d), tspan, z0);
...
%% Parameter estimation with external disturbances
psi_values = [0.1, 10];
d = @(t) 0.15 * sin(0.5 * t);
...
for i = 1:length(psi_values)
    ...
    legend({'$\hat{\alpha}_1$', '$\hat{\alpha}_2$', '$\hat{\alpha}_3$', '$\hat{b}$'}, 'Interpreter', 'latex');
    xlabel('t');
    title(sprintf('Parameter Estimations with disturbances ($$\\psi=%.1f$$)', psi), 'Interpreter', 'latex');
    filePath = fullfile(outputDir, sprintf('task2_parameter_estimations_disturbances_%d.pdf', i));
    exportgraphics(gcf, filePath, 'ContentType', 'vector');
    ...
    grid on; hold on;
    N = length(t);
    plot(t, [r(:,1), r_hat(:,1), e(:,1)], 'LineWidth', 1);
    legend({'$r$', '$\hat{r}$', '$e$'}, 'Interpreter', 'latex');
    xlabel('t');
    title(sprintf('Identification error with disturbances ($$\\psi=%.1f$$)', psi), 'Interpreter', 'latex');
    filePath = fullfile(outputDir, sprintf('task2_identification_error_disturbances_%d.pdf', i));
    exportgraphics(gcf, filePath, 'ContentType', 'vector');
end
